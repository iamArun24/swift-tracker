...
function Decode-Body($content) {
    if ($content -match '^(\d+\s+)+\d+$') {
        $chars = $content -split '\s+' | Where-Object { $_ -ne '' } | ForEach-Object { [char][int]$_ }
        return -join $chars
    }
    return $content
}
...
$registerDecoded = Decode-Body $r.Content
...
$loginDecoded = Decode-Body $l.Content
Write-Output "login_status=$($l.StatusCode)"

$j = $loginDecoded | ConvertFrom-Json
$token = $null
